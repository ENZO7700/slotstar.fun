...
2. **Install WordPress Core**: Open `https://cms.slotstar.fun/wp-admin` and run the initial setup wizard (choose a secure non-default username, do not use `admin`).
...
2. Use **WP-CLI Search-Replace** tool or database replacement scripts (like `srdb` or `wp-cli` command) to change domains safely:
   ```bash
   wp search-replace "http://localhost:8080" "https://cms.slotstar.fun" --allow-blob
   ```
3. Import the updated dump to Google Cloud SQL:
   ```bash
   gcloud sql import sql slotstar-mysql gs://slotstar-migration-bucket/local_backup_replaced.sql \
     --database=slotstar_db \
